...
def comment_callback_for_notification(sender, request=None, comment=None, **kwargs):
    ...
    infodict = {"user": comment.user, "comment":
                comment, "object": comment.content_object}
    ...
    if comment.parent:
        ...
        if comment.parent.user and comment.parent.user != comment.user:
            notification.send(
                [comment.parent.user], "comment_reply_received", infodict)

    else:
        notice_type_suffix = "posted"

    # Notifications of stuff I'm doing
    notification.send([comment.user], "comment_%s" % (
        notice_type_suffix, ), infodict)
    ...
    if friends:
        notification.send((x['friend'] for x in
                           Friendship.objects.friends_for_user(
                           comment.user) if x[
                               'friend'] != comment.parent.user),
                          "comment_friend_%s" % (
                              notice_type_suffix, ), infodict
                          )
    if relationships:
        ...
        notification.send(followers, "comment_friend_%s" % (
            notice_type_suffix, ), infodict)
