...
def comments_fulltree(request, tree_id, *args, **kwargs):
    try:
        comments = get_model().objects.filter_hidden_comments().filter(
            tree_id=tree_id)
        comment = comments[0]
    except IndexError:
        raise Http404("No top level comment found for tree id %s" % tree_id)
    cutoff = getattr(settings, 'MPTT_COMMENTS_FULLTREE_CUTOFF', getattr(
        settings, 'MPTT_COMMENTS_CUTOFF', 3)) + 1
...
def count_for_object(request, content_type_id, object_pk, mimetype='text/plain'):
    ...
    count = str(get_model().objects.filter(
        object_pk=object_pk, content_type=ctype).count())
