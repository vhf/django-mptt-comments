...
class BaseMpttCommentNode(BaseCommentNode):
    ...
    def __init__(self, ctype=None, object_pk_expr=None, object_expr=None, as_varname=None, root_only=False, with_parent=None, reverse=False, flat=False, comment=None, **kwargs):
        ...
        if self.reverse and not self.root_only:
            raise template.TemplateSyntaxError(
                "'reverse' option is only available with root-only ('comments-more' link at toplevel expects normal order)")
    ...
    def handle_token(cls, parser, token):
        ...
        extra_kw = {}
        extra_possible_kw = (
            'root_only', 'flat', 'reverse', 'sort=mostcommented', 'sort=mostrecentreplies')
        ...
        if len(tokens) == 5 or len(tokens) == 7:
            ...
            if len(tokens) == 7:
                if tokens[5] != 'with':
                    raise template.TemplateSyntaxError(
                        "When 6 arguments are given, fifth argument in %r must be 'with' followed by the parent commment wanted" % tokens[0])
...
class MpttCommentInModerationOnlyListNode(BaseMpttCommentWithoutFilteringNode):
    ...
    def get_query_set(self, context):
        qs = super(
            MpttCommentInModerationOnlyListNode, self).get_query_set(context)
...
class MpttCommentListNode(BaseMpttCommentNode):
        ...
        if self.with_parent:
            if self.with_parent in context:
                ...
                try:
                    ...
                    raise template.TemplateSyntaxError(
                        "'%s' doesn't represent a known variable in the context or a tree_id" % self.with_parent)

            if isinstance(parent, int):
                # Interpret parent as a tree_id.
                # Note: in this mode, we include the corresponding root-node
                # too
                self.bottom_level = 0
                ...
                qs = qs.filter(
                    tree_id=parent.tree_id, lft__gt=parent.lft, rght__lt=parent.rght)
    ...
    def render(self, context):
        ...
        context[self.as_varname] = self.get_context_value_from_queryset(
            context, qs)
        use_internal_pagination_system = not getattr(
        ...
        if self.get_offset() > 0 and use_internal_pagination_system:
            ...
            comments_remaining = (comments_remaining - self.get_offset(
            )) > 0 and comments_remaining - self.get_offset() or 0
        ...
        context['collapse_levels_above'] = getattr(
            settings, 'MPTT_COMMENTS_COLLAPSE_ABOVE', 2)
...
class MpttSpecialTreeListNode(MpttCommentListNode):
    ...
    def get_query_set(self, context):
        ...
        if self.sort == 'mostcommented':
            qs = qs.values_list('tree_id', flat=True).filter(
                level=0).order_by('-rght')
        elif self.sort == 'mostrecentreplies':
            qs = qs.values_list('tree_id', flat=True).annotate(
                max_date=Max('submit_date')).order_by('-max_date')
        return qs
...
class MpttCommentCollapseState(template.Node):
    ...
    def render(self, context):
        if not self.varname in context:
            raise template.TemplateSyntaxError(
                "%s is an invalid context variable" % self.varname)

        comment = context[self.varname]
        collapse_levels_above = 'collapse_levels_above' in context and context[
            'collapse_levels_above'] or 1e308
        collapse_levels_below = 'collapse_levels_below' in context and context[
            'collapse_levels_below'] or -1e308
...
register.inclusion_tag('comments/comments_media_css.html', takes_context=True)(
    mptt_comments_media_css)
register.inclusion_tag(
    'comments/comments_media_js.html', takes_context=True)(mptt_comments_media_js)
