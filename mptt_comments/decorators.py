...
def login_required_ajax(view_func):
    ...
    def _wrapped_view(request, *args, **kwargs):
        is_ajax = request.GET.get('is_ajax', request.POST.get(
            'is_ajax')) and '_ajax' or ''
        if is_ajax and not request.user.is_authenticated():
            ...
            # we are not supporting WWW-Authenticate header authentification
            # method
            html = _(
                'Please <a href="%(login_url)s">log-in</a> or <a href="%(register_url)s">create an account</a> before proceeding')
            html = html % {
